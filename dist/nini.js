const niniex = (e, t, n) => {
    ...
    function Get1688Attribute() {
        ...
        if (!r) {
            r = $(".od-gallery-turn-item-wrapper.prepic-active img.od-gallery-img").attr("src");
        }
		...
		if (!s) {
            s = $("button.sku-filter-button.active .label-name").text();
        }
        ...
        if (u.length) {
			...
			if (!u.length){
                u = $(".expand-view-list .expand-view-item");
                price_class = "span.item-price-stock";
                size_class = "span.item-label";
            }
        ...
        pe.ready(function () {
            setTimeout(function () {				
                ...
                if (f() && !j()) {
					...
					pe = $("#J_box_buycart, #J_juValid, .tb-action, .obj-order, .order-button-children, .order-button-wrapper, .od-pc-offer-payment-contain, .action-button-list");
